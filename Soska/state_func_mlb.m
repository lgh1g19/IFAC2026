...
num_joints=3; 
%x_full=input(1:2*(num_joints+num_muscles));
...
[q, q_dot, x]=extract_vals(x_full, num_joints);
...
[M_A, M_B, ~, ~]=tf2ss(w_n^2, [1, 2*w_n, w_n^2]);

%% Calculate f(u,x)

%Assemble lower p elements of function vector
dx=zeros(size(x));
for i = 1:(length(x)/2)
