...
ppts_of_interest=1:9;%[1,3,7]; %[1,2:9];

joints_cell=cell(1, length(fileNames)); %Specifies which joints to fit for each data set
...
for f=1:length(fileNames)
    if length(joints_cell{f})>max_outputs
        max_outputs=length(joints_cell{f});
    end
end
...
out_data_cell_full=cell(max_outputs, length(fileNames));
mean_CI_arr=zeros(max_outputs, length(fileNames));
...
%% Fit IRC to mean response over all data

params_arr=zeros(length(fileNames), num_muscles*(num_inputs+max_outputs+1));
params_arr_lin=zeros(length(fileNames), max_outputs+1);
err_norm_cell=cell(max_outputs, length(fileNames));
err_norm_mean=zeros(max_outputs, length(fileNames));
succ_reps=zeros(length(fileNames),4); %Number of successful repetitions of local optimisation


for f=ppts_of_interest%1:length(fileNames)
    joints=joints_cell{f};
    num_outputs=length(joints);
    ...
    save_fileName=strcat(save_fileName,"numMusc_",string(num_muscles),"_numIn_",string(num_inputs),"_trial_",string(f), ".mat");
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if(~exist(save_fileName, 'file'))
        [in_mean_step, out_mean_step] = format_step_data_general(in_data, q_mean, N_step);

        %Check input on each repeat is the same and remove repeats
        %%%%%%%%%%%%%%%%
        cond=1;
        for test=1:length(in_mean_step)-num_inputs
            cond=cond&&all(in_mean_step{test}==in_mean_step{test+num_inputs});
        end
        if(cond)
            in_mean_step=in_mean_step(1:num_inputs);
            in_data=in_data(:,1:num_inputs);
        else
            warning('Inputs are different on each repeat');
        end
        %%%%%%%%%%%%%%%%

        %Fit model parameters and calculate cross-correlation fitting accuracy
        
        if cross_corr; num_tests=size(q_full{1},2); else; num_tests=1; end
        ...
        for test=1:num_tests
            ...
            for out=1:num_outputs
                q_data_fit{out}=mean(q_full{out}(:,idx),2);
                q_data_val{out}=q_full{out}(:,test);
            end
            [~, out_fit_step] = format_step_data_general(in_data, q_data_fit, N_step);
        ...
        %IRC_ss_params=mean_over_ds(IRC_params_arr_local, num_inputs, num_outputs);
    ...
    % params_arr(f,1:length(IRC_ss_params))=IRC_ss_params; %Not sure taking the mean over all parameters is the best option (though might work better with sat/db equivalent definitions)
    % params_arr_lin(f,1:length(lin_params_mean))=lin_params_mean;

    %%%%% Plot fitting results (needs generalising) %%%%%%%%
        in_data_all=[];
    ...
    for out=1:num_outputs
    q_off(out)=find_offset(q_full{out}, in_data);
    end
...
%%
%IRC_ss_params=mean_over_ds(IRC_ss_params_full, num_inputs, num_outputs);
